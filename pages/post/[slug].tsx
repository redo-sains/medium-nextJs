...
function Post({ post }: Props) {
  ...
  const { title, description, author, _createdAt, body } = post;
  ...
  return (
    <main>
      ...
      <article className="max-w-3xl mx-auto p-5">
        ...
        <h2 className="text-xl font-light text-gray-500 mb-5">{description}</h2>
        <div className="flex items-center space-x-2">
          ...
          <p className="font-extralight text-sm">
            Blog post by <span className="text-green-500">{author.name}</span> -
            published at {new Date(_createdAt).toLocaleDateString()}
          </p>
        </div>
        <div className="mt-10">
          <PortableText
      ...
      <div className="flex flex-col p-10 my-10 max-w-2xl mx-auto shadow-yellow-500 shadow space-y-2">
        ...
        {post.comments.map((d) => {
          return (
            <div key={d._id}>
              <p>
                <span className="text-yellow-500">{d.name} : </span>
                {d.comment}
              </p>
            </div>
          );
